...
import ReferralNotice from "./ReferralNotice"
...
export default function Referral({setPremiumEndDate} : ReferralProps) {
    const [refCode, setRefCode] = useState<string>("")
    const [notice, setNotice] = useState(<></>)

    function giveNotice(noticeType: String, noticeInfo: String) {
      setNotice(
          <ReferralNotice noticeType={noticeType} noticeInfo={noticeInfo}/>
      )
      setTimeout(()=>{
          setNotice(<></>)
      }, 2000)
    ...
    const applyReferral = (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      const referralCode = refCode.trim()
